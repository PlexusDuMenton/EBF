...
function CHoldoutGameRound:Begin()
	...
	print (PlayerNumber)
	local GoldMultiplier = ((PlayerNumber+0.56)/1.8)
	print (GoldMultiplier)




	self._nGoldRemainingInRound = self._nMaxGold * GoldMultiplier 
...
function CHoldoutGameRound:GetXPPerCoreUnit()
	local PlayerNumber = PlayerResource:GetTeamPlayerCount()
	local ExpMultiplier = 0.125*((0.3*PlayerNumber-3)*(0.3*PlayerNumber-3)*(0.3*PlayerNumber-3))+3.3 * 1.2
	if self._nCoreUnitsTotal == 0 then
		return 0
	else
		return math.floor( (self._nFixedXP * ExpMultiplier) / self._nCoreUnitsTotal )
	end
end
