...
function doom_doom_ebf:OnSpellStart()
	...
	if self:GetCaster():HasScepter() then
		...
		if hTarget == nil or ( hTarget ~= nil and ( not hTarget:TriggerSpellAbsorb( self ) ) ) then
			...
			for i=1,8,1 do
                self:ApplyAOE("particles/econ/items/shadow_fiend/sf_fire_arcana/sf_fire_arcana_shadowraze.vpcf",nil,location,250,self.damage,self:GetAbilityDamageType(), "modifier_doom_doom_ebf", 9999)
			...
			for i=1,8,1 do
                self:ApplyAOE("particles/econ/items/shadow_fiend/sf_fire_arcana/sf_fire_arcana_shadowraze.vpcf",nil,newloca,250,self.damage,self:GetAbilityDamageType(), "modifier_doom_doom_ebf", 9999)
			...
			for i=1,8,1 do
                self:ApplyAOE("particles/econ/items/shadow_fiend/sf_fire_arcana/sf_fire_arcana_shadowraze.vpcf",nil,newlocb,250,self.damage,self:GetAbilityDamageType(), "modifier_doom_doom_ebf", 9999)
                newlocb = newlocb + newdirb * 150
            end
		end
	else
		hTarget:AddNewModifier( self:GetCaster(), self, "modifier_doom_doom_ebf", { duration = 9999 } )
	end
end
