		...
		if caster.fervor_target == target then
			...
			if caster:HasModifier(modifier) then
				...
				if stack_count < max_stacks then
					...
					if chance*(stack_count + 1) > math.random(100 - ability.prng) and not caster.stuntattack then
					...
					if chance*stack_count > math.random(100 - ability.prng) and not caster.stuntattack then
						caster:StartGestureWithPlaybackRate(ACT_DOTA_ATTACK, (2*caster:GetAttacksPerSecond()))
