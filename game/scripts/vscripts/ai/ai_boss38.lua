	...
	if not thisEntity:IsChanneling() then
		if thisEntity.rewind:IsFullyCastable() then
			...
			if hpDeficitent > thisEntity:GetMaxHealth()*0.10 then
				...
				if not target then target = AICore:WeakestEnemyHeroInRange( thisEntity, thisEntity.rewind:GetCastRange(), false) end
				if target then
		...
		or (thisEntity.yesteryear:IsFullyCastable() and ( thisEntity:GetMana() < thisEntity.overtime:GetManaCost() or thisEntity:GetMana() < thisEntity.juncture:GetManaCost() ) )
